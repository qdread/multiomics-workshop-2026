...
- Combine the three pre-transformed omics datasets together and do any final pre-processing
- Do partial least squares analysis on each pair of omics to get correlation parameters
- Fit an initial DIABLO model
- Cross-validate the initial model to determine the optimal number of components
- Cross-validate the model again to determine the optimal number of features per component
- Fit the final DIABLO model
- Make a bunch of plots to make sure the model is valid and explore patterns in the data that the model reveals
- One last cross-validation to assess the final model performance
